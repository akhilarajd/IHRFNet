...
def main():
    ...
    noise_level_img = 10                # set AWGN noise level for noisy image
    noise_level_model = noise_level_img  # set noise level for model
    model_name = 'IHRFNet_color'           # set denoiser model, 'IHRFNet_color' | 'IHRFNet_color'
    testset_name = 'CUrban100'               # set test set,  'bsd68' | 'cbsd68' | 'set12'
    ...
    result_name = testset_name + '_' + task_current + '_' + model_name + '_' + str(noise_level_img)
    ...
    from models.network import IHRFNet as net
    model = net(in_nc=n_channels, out_nc=n_channels, nc=128, bias=False)
    ...
    for idx, img in enumerate(L_paths):
        ...
        #util.imsave(img_L, os.path.join(E_path, img_name+ext))
        #util.imsave(img_E, os.path.join(E_path, f"denoised_{img_name}{ext}"))
        util.imsave(img_L, os.path.join(E_path, f"noisy_{img_name}{ext}"))

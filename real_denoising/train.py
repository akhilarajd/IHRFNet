...
model_restoration = IHRFNet(in_nc=3, out_nc=3, nc=128, bias=False)
...
val_loader = DataLoader(dataset=val_dataset, batch_size=8, shuffle=False, num_workers=16, drop_last=False, pin_memory=True)
...
for epoch in range(start_epoch, opt.OPTIM.NUM_EPOCHS + 1):
    ...
    for i, data in enumerate(tqdm(train_loader), 0):
        ...
        loss = criterion1(torch.clamp(restored, 0, 1), target) + 0.1 * criterion2(torch.clamp(restored, 0, 1), target)
